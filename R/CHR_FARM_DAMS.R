...
if(dam_list) {
  ...
  NSW_CHR_UWS_aggregated <- st_read(dsn = paste0(data_dir, 
                                                 'CHR_DPEW_Input_Water_Source_Data.gpkg'),
  ...
  NSW_UWS_CHR_WS <- st_read(dsn   = paste0(data_dir, 'CHR_DPEW_Input_Water_Source_Data.gpkg'),
  ...
  # 
  # 
  # NSW_CHR_WS_Harc_Areas <- NSW_CHR_WS_Harc_intersect_sf %>%  
  #   
  #   dplyr::mutate(m2            = st_area(geometry),
  #                 Harc_CHR_Ha   = units::set_units(m2, value = ha),
  #                 Harc_CHR_Ha   = drop_units(Harc_CHR_Ha),
  #                 
  #                 Harc_CHR_Sqkm = units::set_units(m2, value = km2),
  #                 Harc_CHR_Sqkm = drop_units(Harc_CHR_Sqkm))
  # 
  # 
  # NSW_CHR_WS_Harc_Areas_df <- NSW_CHR_WS_Harc_Areas %>% as_tibble() %>% 
  #   select(-geometry, -m2)
  
  
  ## This is the full intersect with HARC and CHR_WS
  ## In the loop, the corresponding intersect is between DAM and CHR_WS
  ## Save that out as a table too, for cross checking
  # write_csv(NSW_CHR_WS_Harc_Areas_df,
  #           paste0(hydro_out, 'dams/', 'NSW_CHR_WS_Harc_Areas_df.csv'))
  ...
  ## LUT
  NSW_CHR_Water_Sources_LUT <- 
    read_csv(paste0(data_dir, 
                    'NSW_CHR_Water_Sources_LUT.csv')) %>% 
    select(CHR_Water_Source, UNREGULATED_WATER_SOURCE) %>% 
    distinct()
